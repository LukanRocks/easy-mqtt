...
import { DynsecClient, DynsecError, DynsecProtocolError, DynsecTimeoutError, resolveAccess, type Client, type Group, type Role } from '@easy-mqtt/dynsec'
...
const auth_routes = new Hono<Env>()
  .post('/login', zValidator('json', loginSchema), async (c) => {
    ...
    try {
      ...
      return c.json({ error: 'invalid username, password, or no access to $CONTROL' }, 401)
    }
    const sealed = await sealSession({ username, password, iat: Math.floor(Date.now() / 1000) }, config.sessionSecret)
...
const access_routes = new Hono<Env>().use('*', auth).get('/', zValidator('query', accessQuerySchema), async (c) => {

...
export function RoleDetailPage({ rolename }: { rolename: string }) {
  ...
  const removeAcl = useMutation({
    mutationFn: (acl: { acltype: AclType; topic: string }) =>
      api.roles.removeAcl(rolename, acl.acltype, acl.topic),
    onSuccess: () => {
      refresh();
                    ...
                    {acls.length ? (
                      acls.map((acl) => (
                        <TableRow key={`${acl.acltype}:${acl.topic}`}>
                          ...
                          <TableCell>
                            <Badge variant={acl.allow ? "success" : "destructive"}>
                          ...
                          <TableCell className="text-right">
                            <Button
                              ...
                              title="Remove ACL"
                              onClick={() =>
                                removeAcl.mutate({ acltype: acl.acltype, topic: acl.topic })
                              }
                            >
